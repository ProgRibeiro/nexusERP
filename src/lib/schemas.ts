...
export const quoteCreateSchema = z.object({
  ...
  proposalType: z.enum(["AVULSA", "PREVENTIVA", "LICITACAO"]).optional(),
  procurementNumber: z.string().trim().max(120).optional(),
  contractingAgency: z.string().trim().max(180).optional(),
  biddingNumber: z.string().trim().max(120).optional(),
  referenceBase: z.string().trim().max(80).optional(),
  referenceMonth: z.string().trim().max(30).optional(),
  publicBudgetSource: z.string().trim().max(120).optional(),
  deliveryTerm: z.string().trim().max(120).optional(),
