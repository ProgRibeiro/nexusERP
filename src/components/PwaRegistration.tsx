...
export default function PwaRegistration() {
  ...
  useEffect(() => {
    ...
    if (process.env.NODE_ENV !== "production") {
      ...
      if ("caches" in window) {
        void caches.keys().then((keys) =>
          Promise.all(keys.filter((key) => key.startsWith("nx-erp-")).map((key) => caches.delete(key))),
        );
      }
      return () => {
        window.clearTimeout(initialTimer);
    ...
    const register = async () => {
      try {
        ...
        console.warn(
          "Não foi possível registrar o aplicativo instalável.",
          error,
        );
      }
    };
  ...
  return (
    <>
      {offline && (
        <div
          className="fixed inset-x-3 top-[max(.75rem,env(safe-area-inset-top))] z-[100] mx-auto flex max-w-md items-center gap-3 rounded-2xl border border-amber-300 bg-amber-50 px-4 py-3 text-amber-900 shadow-xl print:hidden dark:border-amber-800 dark:bg-amber-950 dark:text-amber-100"
          role="status"
        >
          <WifiOff size={18} className="shrink-0" />
          <div className="min-w-0 flex-1">
            <p className="text-xs font-black">Modo sem conexão</p>
            <p className="text-[10px] leading-relaxed opacity-80">
              Os dados do servidor voltarão quando a rede for restabelecida.
            </p>
          </div>
        </div>
      )}
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-[10px] text-zinc-500">
              Reabra o ERP para usar a versão mais recente.
            </p>
          </div>
          <button
            type="button"
