...
import { BriefcaseBusiness, CalendarDays, CheckCircle2, CircleDollarSign, Loader2, Plus, Search, Users } from "lucide-react";
...
export default function PrestadoresTab() {
  ...
  return <div className="space-y-5 text-zinc-100">
    ...
    <Modal isOpen={modal} onClose={()=>setModal(false)} title="Cadastrar prestador"><form className="space-y-4" onSubmit={async e=>{e.preventDefault();setBusy("new");const r=await createProvider(form);if(r.success){toast("Prestador cadastrado.","success");setModal(false);setForm({name:"",cnpj:"",phone:"",email:"",notes:""});await load();}else toast(r.error||"Erro ao cadastrar.","error");setBusy("");}}><Input label="Nome / Razão social" required value={form.name} onChange={e=>setForm({...form,name:e.target.value})}/><Input label="CPF / CNPJ" required value={form.cnpj} onChange={e=>setForm({...form,cnpj:e.target.value})}/><div className="grid grid-cols-2 gap-3"><Input label="Telefone" required value={form.phone} onChange={e=>setForm({...form,phone:e.target.value})}/><Input label="E-mail" type="email" required value={form.email} onChange={e=>setForm({...form,email:e.target.value})}/></div><Input label="Observações internas" value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})}/><div className="flex justify-end"><Button type="submit" disabled={busy==="new"}>{busy==="new"?<Loader2 className="animate-spin"/>:<Plus size={15}/>} Cadastrar</Button></div></form></Modal>
  </div>;
}
