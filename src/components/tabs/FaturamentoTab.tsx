...
import { BillingQueueItem, getBillingQueue, getInvoices, processBilling } from "@/app/actions/billingActions";
...
export default function FaturamentoTab() {
  ...
  const filteredRows = useMemo(() => {
    ...
    if (!term) return rows;
    return rows.filter((item) => item.code.toLowerCase().includes(term) || item.legalName.toLowerCase().includes(term) || item.clientDocument.includes(term));
  }, [rows, search]);
  ...
  const saveEditing = () => {
    ...
    setRows((current) => current.map((row) => row.id === editing.id ? { ...editing, clientDocument: document, cnae: cleanCnae(editing.cnae), cep: editing.cep.replace(/\D/g, ""), missingFields: missing } : row));
    setEditing(null);
    toast("Dados ajustados no espelho desta exportação.", "success");
  };
  ...
  const exportSpreadsheet = async () => {
    ...
    try {
      ...
      instructions.addRow(["5. Depois da emissão externa, retorne ao ERP e use Registrar nota emitida."]);
  ...
  const registerExternalInvoice = async (event: React.FormEvent) => {
    ...
    setActionLoading(true);
    try {
      const result = await processBilling({ osId: registering.id, invoiceCode: invoiceCode.trim(), totalValue: invoiceValue, taxPercent: Number(taxPercent) || 0, installments: Number(installments) || 1, paymentMethod, notes: `Nota emitida no sistema externo. Valor original da OS: ${formatCurrency(registering.value)}.`, userId: user?.id || "" });
      if (!result.success) {
  ...
  return (
    <div className="space-y-6 pb-10">
      ...
      {activeTab === "mirror" ? <Card className="space-y-4 p-0 overflow-hidden">
        <div className="flex flex-col gap-3 border-b border-zinc-100 p-5 sm:flex-row sm:items-center sm:justify-between dark:border-zinc-800"><div><h3 className="text-sm font-black">Dados para a planilha de notas</h3><p className="text-xs text-zinc-500">Os campos seguem a mesma ordem do arquivo enviado.</p></div><div className="w-full sm:w-80"><Input placeholder="Buscar OS, tomador ou documento" icon={<Search size={14} />} value={search} onChange={(e) => setSearch(e.target.value)} /></div></div>
        {!rows.length ? <div className="py-16 text-center"><CheckCircle2 size={32} className="mx-auto text-emerald-500" /><p className="mt-3 text-sm font-bold">Nenhuma OS aguardando nota</p><p className="text-xs text-zinc-500">Quando uma OS for concluída, ela aparecerá aqui.</p></div> : <>
          <div className="grid grid-cols-1 gap-3 p-3 sm:p-4 xl:hidden">{filteredRows.map((row) => <article key={row.id} className="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-800 dark:bg-zinc-900"><div className="flex items-start gap-3"><input className="mt-1" type="checkbox" checked={row.selected} onChange={(e) => setRows((current) => current.map((item) => item.id === row.id ? { ...item, selected: e.target.checked } : item))} /><div className="min-w-0 flex-1"><div className="flex flex-wrap items-start justify-between gap-2"><div><strong className="text-sm">{row.code}</strong><p className="text-[10px] text-zinc-500">Concluída {row.completedAt ? formatDate(row.completedAt) : ""}</p></div>{row.missingFields.length ? <span className="flex items-center gap-1 rounded-full bg-orange-50 px-2 py-1 text-[10px] font-bold text-orange-600 dark:bg-orange-950/30"><AlertTriangle size={11} /> Revisar {row.missingFields.length}</span> : <span className="flex items-center gap-1 rounded-full bg-emerald-50 px-2 py-1 text-[10px] font-bold text-emerald-600 dark:bg-emerald-950/30"><Check size={11} /> Pronta</span>}</div><h4 className="mt-3 truncate text-sm font-bold">{row.legalName}</h4><p className="mt-1 font-mono text-xs text-zinc-500">{cleanDocument(row.clientDocument)}</p><div className="mt-4 grid grid-cols-2 gap-3 text-xs sm:grid-cols-4"><div><span className="block text-[9px] font-bold uppercase text-zinc-400">Valor OS</span><strong>{formatCurrency(row.value)}</strong></div><div><span className="block text-[9px] font-bold uppercase text-zinc-400">CNAE</span><span>{row.cnae || "Principal"}</span></div><div><span className="block text-[9px] font-bold uppercase text-zinc-400">CEP / Nº</span><span>{row.cep || "—"} / {row.addressNumber || "—"}</span></div><div><span className="block text-[9px] font-bold uppercase text-zinc-400">E-mail</span><span className="block truncate">{row.email}</span></div></div><div className="mt-4 rounded-lg bg-zinc-50 p-3 text-[11px] text-zinc-600 dark:bg-zinc-800 dark:text-zinc-300">{row.description || "Descrição opcional não informada"}</div><div className="mt-4 flex flex-wrap justify-end gap-2"><Button size="sm" variant="secondary" onClick={() => setEditing({ ...row })}><Pencil size={13} /> Revisar</Button><Button size="sm" variant="secondary" onClick={() => void copyRow(row)}><Clipboard size={13} /> Copiar</Button>{hasPermission("faturamento.write") && <Button size="sm" variant="primary" onClick={() => openRegister(row)}><Send size={13} /> Registrar NF</Button>}</div></div></div></article>)}</div>
          <div className="hidden overflow-x-auto xl:block"><table className="min-w-[1500px] w-full text-left"><thead><tr className="bg-zinc-50 text-[10px] uppercase tracking-wide text-zinc-500 dark:bg-zinc-950/50"><th className="px-4 py-3 w-12"><input type="checkbox" checked={rows.length > 0 && rows.every((row) => row.selected)} onChange={(e) => setRows((current) => current.map((row) => ({ ...row, selected: e.target.checked })))} /></th><th className="px-3 py-3">OS / Situação</th><th className="px-3 py-3">Razão social ou tomador *</th><th className="px-3 py-3">CPF/CNPJ</th><th className="px-3 py-3">Valor</th><th className="px-3 py-3">Descrição</th><th className="px-3 py-3">CNAE</th><th className="px-3 py-3">E-mail</th><th className="px-3 py-3">CEP</th><th className="px-3 py-3">Número</th><th className="px-4 py-3 text-right">Ações</th></tr></thead><tbody className="divide-y divide-zinc-100 text-xs dark:divide-zinc-800">{filteredRows.map((row) => <tr key={row.id} className="align-top hover:bg-blue-50/30 dark:hover:bg-blue-950/10"><td className="px-4 py-4"><input type="checkbox" checked={row.selected} onChange={(e) => setRows((current) => current.map((item) => item.id === row.id ? { ...item, selected: e.target.checked } : item))} /></td><td className="px-3 py-4"><strong className="block">{row.code}</strong><span className="text-[10px] text-zinc-500">Concluída {row.completedAt ? formatDate(row.completedAt) : ""}</span>{row.missingFields.length ? <span className="mt-1 flex items-center gap-1 text-[10px] font-bold text-orange-600"><AlertTriangle size={11} /> Revisar {row.missingFields.length}</span> : <span className="mt-1 flex items-center gap-1 text-[10px] font-bold text-emerald-600"><Check size={11} /> Pronta</span>}</td><td className="max-w-64 px-3 py-4 font-semibold">{row.legalName}</td><td className="px-3 py-4 font-mono">{cleanDocument(row.clientDocument)}</td><td className="px-3 py-4 font-bold">{formatCurrency(row.value)}</td><td className="max-w-72 px-3 py-4 text-zinc-600 dark:text-zinc-400">{row.description || <span className="italic text-zinc-400">Opcional</span>}</td><td className="px-3 py-4 font-mono">{row.cnae || <span className="text-zinc-400">Principal</span>}</td><td className={`max-w-64 px-3 py-4 ${row.email.endsWith("@importado.local") ? "text-orange-600" : ""}`}>{row.email}</td><td className="px-3 py-4 font-mono">{row.cep || "—"}</td><td className="px-3 py-4">{row.addressNumber || "—"}</td><td className="px-4 py-4"><div className="flex justify-end gap-1"><button onClick={() => setEditing({ ...row })} className="rounded-lg border border-zinc-200 p-2 text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800" title="Revisar dados"><Pencil size={14} /></button><button onClick={() => void copyRow(row)} className="rounded-lg border border-zinc-200 p-2 text-zinc-600 hover:bg-zinc-100 dark:border-zinc-700 dark:hover:bg-zinc-800" title="Copiar linha"><Clipboard size={14} /></button>{hasPermission("faturamento.write") && <button onClick={() => openRegister(row)} className="rounded-lg bg-blue-600 p-2 text-white hover:bg-blue-700" title="Registrar nota emitida"><Send size={14} /></button>}</div></td></tr>)}</tbody></table></div>
        </>}
      </Card> : <Card className="p-0 overflow-hidden"><div className="border-b border-zinc-100 p-5 dark:border-zinc-800"><h3 className="text-sm font-black">Histórico de notas registradas</h3><p className="text-xs text-zinc-500">Notas emitidas externamente e confirmadas no ERP.</p></div>{invoices.length ? <Table headers={["Nota", "Tomador", "OS", "Valor", "Imposto", "Registro", "Arquivo"]}>{invoices.map((invoice) => <TableRow key={invoice.id}><TableCell className="font-bold">{invoice.code}</TableCell><TableCell>{invoice.client?.name || "—"}</TableCell><TableCell>{invoice.serviceOrder?.code || "—"}</TableCell><TableCell className="font-bold">{formatCurrency(invoice.value)}</TableCell><TableCell>{formatCurrency(invoice.taxValue)}</TableCell><TableCell>{formatDate(invoice.createdAt)}</TableCell><TableCell>{invoice.pdfUrl ? <a className="flex items-center gap-1 font-bold text-blue-600" href={invoice.pdfUrl}><FileText size={13} /> Arquivo</a> : "—"}</TableCell></TableRow>)}</Table> : <div className="py-14 text-center text-xs text-zinc-500">Nenhuma nota externa registrada.</div>}</Card>}

      <Modal isOpen={Boolean(editing)} onClose={() => setEditing(null)} title="Revisar dados do espelho" size="xl">{editing && <div className="space-y-5"><div className="rounded-xl bg-blue-50 p-4 text-xs text-blue-800 dark:bg-blue-950/30 dark:text-blue-300"><strong>{editing.code}</strong> · As alterações abaixo valem para esta fila e para a planilha exportada.</div><div className="grid grid-cols-1 gap-4 md:grid-cols-2"><Input label="Razão social ou Nome do tomador *" value={editing.legalName} onChange={(e) => updateEditing("legalName", e.target.value)} /><Input label="CPF/CNPJ *" value={editing.clientDocument} onChange={(e) => updateEditing("clientDocument", e.target.value)} /><Input label="Valor *" type="number" value={editing.value} onChange={(e) => setEditing((current) => current ? { ...current, value: Number(e.target.value) || 0 } : current)} /><Input label="CNAE (somente números, opcional)" value={editing.cnae} onChange={(e) => updateEditing("cnae", e.target.value)} /><Input label="E-mail" type="email" value={editing.email} onChange={(e) => updateEditing("email", e.target.value)} /><div className="grid grid-cols-2 gap-3"><Input label="CEP" value={editing.cep} onChange={(e) => updateEditing("cep", e.target.value)} /><Input label="Número" value={editing.addressNumber} onChange={(e) => updateEditing("addressNumber", e.target.value)} /></div></div><Input label="Descrição do serviço (opcional)" value={editing.description} onChange={(e) => updateEditing("description", e.target.value)} /><div className="flex justify-end gap-2 border-t border-zinc-100 pt-4 dark:border-zinc-800"><Button variant="secondary" onClick={() => setEditing(null)}>Cancelar</Button><Button variant="primary" onClick={saveEditing}>Salvar no espelho</Button></div></div>}</Modal>

      <Modal isOpen={Boolean(registering)} onClose={() => setRegistering(null)} title="Registrar nota emitida externamente" size="lg">{registering && <form onSubmit={registerExternalInvoice} className="space-y-5"><div className="rounded-xl border border-emerald-100 bg-emerald-50 p-4 text-xs text-emerald-800 dark:border-emerald-950 dark:bg-emerald-950/20 dark:text-emerald-300"><strong>{registering.code} · {registering.legalName}</strong><p className="mt-1">Use esta etapa somente depois que a nota já tiver sido emitida no sistema externo. O ERP registrará a nota e criará as contas a receber pelo valor informado abaixo.</p></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><Input label="Número / código da NF emitida *" required placeholder="Ex: 12345" value={invoiceCode} onChange={(e) => setInvoiceCode(e.target.value)} /><Input label="Valor efetivo da nota (R$) *" required min="0.01" step="0.01" type="number" value={registeredValue} onChange={(e) => setRegisteredValue(e.target.value)} /></div><div className="rounded-xl bg-zinc-50 p-3 text-xs dark:bg-zinc-800"><span className="text-zinc-500">Valor original calculado pela OS: </span><strong>{formatCurrency(registering.value)}</strong></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-3"><Input label="Imposto total (%)" type="number" value={taxPercent} onChange={(e) => setTaxPercent(e.target.value)} /><Select label="Parcelas" value={installments} onChange={(e) => setInstallments(e.target.value)} options={[{ value: "1", label: "1 parcela" }, { value: "2", label: "2 parcelas" }, { value: "3", label: "3 parcelas" }]} /><Select label="Forma de pagamento" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value)} options={[{ value: "PIX", label: "PIX" }, { value: "BOLETO", label: "Boleto" }, { value: "TRANSFERENCIA", label: "Transferência" }, { value: "CARTAO", label: "Cartão" }]} /></div><div className="flex justify-end gap-2 border-t border-zinc-100 pt-4 dark:border-zinc-800"><Button type="button" variant="secondary" onClick={() => setRegistering(null)}>Cancelar</Button><Button type="submit" variant="success" loading={actionLoading}><Receipt size={14} /> Confirmar NF e valor</Button></div></form>}</Modal>
    </div>
  );
}
